"""Split-20Newsgroups 文本数据集（DistilBERT 预 tokenize 版本）。

复用 split_newsgroups.py 的任务分组，但不预计算特征——
...
from src.data_module.dataset.split_newsgroups import TASK_GROUPS, _load_raw_texts

logger = logging.getLogger(__name__)

REPORTED_INPUT_DIM = 768   # DistilBERT hidden_size，告知 trainer/model
N_CLASSES = 20
N_TASKS = 5
CLASSES_PER_TASK = 4
DEFAULT_MAX_LEN = 128
...
class _TokenizedTextDataset(Dataset):
    ...
    def __init__(
        ...
        self.input_ids = input_ids          # (N, L) LongTensor
        self.attention_mask = attention_mask  # (N, L) LongTensor
...
def _tokenize_texts(
    ...
    logger.info(f"加载 tokenizer：{model_name}（max_length={max_length}）")
...
@register_dataset("split_newsgroups_text")
class SplitNewsgroupsText(BaseContinualDataset):
    """Split-20Newsgroups（原始文本，预 tokenize 后缓存）。

    供可微调 DistilBERT encoder 使用；DataLoader 返回
    ((input_ids, attn_mask), label) 格式，无需在 model 外 tokenize。
    """
    ...
    def _setup(self) -> None:
        ...
        if train_cache.exists() and val_cache.exists():
            logger.info("加载 Split-Newsgroups tokenized 缓存...")
            ...
            logger.info("首次运行，tokenize 20Newsgroups 文本（约 1-2 分钟）...")
            ...
            logger.info("Tokenized 缓存已保存")

        self.INPUT_DIM = REPORTED_INPUT_DIM
        logger.info(
            f"构建 SplitNewsgroupsText：{N_TASKS} 任务，"
        ...
        logger.info("SplitNewsgroupsText 构建完成")
